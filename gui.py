...
def update_enrolled_courses():
    while True:
        ...
        main_window.write_event_value("Update-Menu")
        time.sleep(8)
...
def create_scraping_thread(site: str):
    ...
    try:
        ...
        while getattr(scraper, f"{code_name}_length") == 0:
            time.sleep(0.1)  # Tránh chờ đợi bận rộn
        if getattr(scraper, f"{code_name}_length") == -1:
            raise Exception(f"Lỗi ở: {site}")
        ...
        error_message = getattr(scraper, f"{code_name}_error", "Lỗi không xác định")
        main_window.write_event_value(
            "Error", f"{error_message}|:|Lỗi không xác định ở: {site} {VERSION}"
        )
    finally:
...
def scrape():
    try:
        ...
        main_window["se_c"].update(
            value=f"Đã đăng ký thành công: {udemy.successfully_enrolled_c}"
        )
        main_window["as_c"].update(
            value=f"Số tiền đã tiết kiệm: {round(udemy.amount_saved_c,2)} {udemy.currency.upper()}"
        )
        main_window["ae_c"].update(
            value=f"Đã đăng ký trước đó: {udemy.already_enrolled_c}"
        )
        main_window["e_c"].update(value=f"Khóa học hết hạn: {udemy.expired_c}")
        main_window["ex_c"].update(value=f"Khóa học bị loại trừ: {udemy.excluded_c}")

    except:
        e = traceback.format_exc()
        main_window.write_event_value(
            "Error",
            f"{e}\n\nPhiên bản:{VERSION}\nLink:{getattr(udemy, 'link', 'None')}\nTiêu đề:{getattr(udemy, 'title','None')}|:|Lỗi g100",
        )
...
try:
    if udemy.settings["stay_logged_in"]["auto"]:
        ...
        raise LoginException("Không tìm thấy đăng nhập đã lưu")
        ...
        if event in (None,):
            ...
            try:
                ...
                try:
                    ...
                    print(e)
                    sg.popup_auto_close(
                        "Hãy chắc chắn bạn đã đăng nhập vào udemy.com trong trình duyệt chrome",
                        title="Lỗi",
                ...
                e = traceback.format_exc()
                sg.popup_scrolled(e, title=f"Lỗi không xác định {VERSION}")
...
title_ex_lo = [
    ...
    [sg.Text("Từ khóa trong các dòng mới\nKhông phân biệt chữ hoa chữ thường")],
]
...
advanced_tab = [
    ...
    [
        sg.Checkbox(
            "Lưu khóa học đã đăng ký vào txt",
            key="save_txt",
    ...
    [
        sg.Checkbox(
            "Đăng ký chỉ trong các khóa học giảm giá",
            key="discounted_only",
